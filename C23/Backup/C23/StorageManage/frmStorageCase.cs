    ...
    public partial class frmStorageCase : Form
    {
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols; i++)
            {
                ...
                dgvStorageCaseInfo.Columns[i].ReadOnly = true;   
            }
        }
        #endregion
        private void BindData()
        {
            ...
            for (i = 0; i < dgvStorageCaseInfo.Columns.Count - 1; i++)
            {
                dgvStorageCaseInfo.Columns[i].SortMode = DataGridViewColumnSortMode.NotSortable;
        ...
        private void tsbtnLook_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataSet myds = boperate.getds(M_str_sql + " where WareName like '%" + tstxtKeyWord.Text.Trim() + "%'", M_str_table);
                    if (myds.Tables[0].Rows.Count > 0)
                        dgvStorageCaseInfo.DataSource = myds.Tables[0];
                    else
                        MessageBox.Show("没有要查找的相关记录！");
                ...
                {
                    DataSet myds = boperate.getds(M_str_sql + " where StorageType ='" + tstxtKeyWord.Text.Trim() + "'", M_str_table);
                    if (myds.Tables[0].Rows.Count > 0)
                        dgvStorageCaseInfo.DataSource = myds.Tables[0];
                ...
                {
                    DataSet myds = boperate.getds(M_str_sql + " where LocationName like '%" + tstxtKeyWord.Text.Trim() + "%'", M_str_table);
                    if (myds.Tables[0].Rows.Count > 0)
                        dgvStorageCaseInfo.DataSource = myds.Tables[0];
                    else
                        MessageBox.Show("没有要查找的相关记录！");
        ...
        private void dgvStorageCaseInfo_DoubleClick(object sender, EventArgs e)
        {
            if (this.dgvStorageCaseInfo.ReadOnly == true) //判断如果是在进货单中生成的窗体则不响应DataGrid的双击事件
            {
                ...
                if (select == 0)
                {
                    C23.SellManage .frmSellTableT.inputTextDataStorage[0] = sendArray[0];
                    C23.SellManage.frmSellTableT.inputTextDataStorage[1] = sendArray[1];
                }
                ...
                if (select == 3)
                {
                    C23.StorageManage.frmMateReT.inputTextDataStorage[0] = sendArray[0];
                    C23.StorageManage.frmMateReT.inputTextDataStorage[1] = sendArray[1];
                }
        ...
        {
            for (i = 0; i < dgvStorageCaseInfo.Columns.Count; i++)
            {
                if (dgvStorageCaseInfo.Columns[i].ValueType.ToString() == "System.Decimal")
                {
                    dgvStorageCaseInfo.Columns[i].DefaultCellStyle.Format = "N";
                    dgvStorageCaseInfo.Columns[i].DefaultCellStyle.Alignment = DataGridViewContentAlignment.BottomRight;
