...
namespace C23.StockManage
{
    public partial class frmStockTable : Form
    {
        ...
        protected string M_str_sql = " select   cast(0   as   bit)   as   复选框,SN as 序号,StockID as 进货单号,WareID as 品号,WareName as 品名,"
             + "Spec as 规格,Unit as 单位,PurOrdersID as 采购单号,PurOrdersSN as 采购单序号,PCount as 采购数量,StockCount as 进货数量,CheckCount as 验收数量,PUnitPrice as 采购单价,"
        ...
        private void dgvStateControl()
        {
            ...
            for (i = 0; i < numCols; i++)
            {
                ...
                if (i!=0 && i != 3  && i != 7 && i!=11 && i!=12 && i !=24 && i!=25)
                {
        ...
        private void dgvStockInfo_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgvStockInfo.CurrentCell.ColumnIndex == 25)
            {
            ...
            dtpStockDate.Text = Convert.ToString(dgvStockInfo[26, dgvStockInfo.CurrentCell.RowIndex].Value).Trim();
            cboxStokerID.Text = Convert.ToString(dgvStockInfo[20, dgvStockInfo.CurrentCell.RowIndex].Value).Trim();
            txtStokerName.Text = Convert.ToString(dgvStockInfo[21, dgvStockInfo.CurrentCell.RowIndex].Value).Trim();
            cboxPur.Text = Convert.ToString(dgvStockInfo[22, dgvStockInfo.CurrentCell.RowIndex].Value).Trim();
            cboxAcceptor.Text = Convert.ToString(dgvStockInfo[23, dgvStockInfo.CurrentCell.RowIndex].Value).Trim();
            txtTotalCount.Text = Convert.ToString(dgvStockInfo[17, dgvStockInfo.CurrentCell.RowIndex].Value).Trim();
            txtNoTax.Text = Convert.ToString(dgvStockInfo[18, dgvStockInfo.CurrentCell.RowIndex].Value).Trim();
            txtTax.Text = Convert.ToString(dgvStockInfo[19, dgvStockInfo.CurrentCell.RowIndex].Value).Trim();
        }
        private void setStorageData()
        {
            dgvStockInfo.Rows[dgvStockInfo.CurrentCell.RowIndex].Cells[24].Value = inputTextDataStorage[0];

        }
        private void setLocationData()
        {
            dgvStockInfo.Rows[dgvStockInfo.CurrentCell.RowIndex].Cells[25].Value = inputTextDataLocation[0];
        }
        #endregion
                ...
                if (i == 0)
                {
                    for (i = 0; i < dgvStockInfo.Rows.Count; i++)
                    {
                        if ((bool)dgvStockInfo.Rows[i].Cells[0].EditedFormattedValue == true)
                        {
                            ...
                            string varWareID, varWareName, varSpec, varUnit, varStockID, varSCheckCount, varPUnitPrice,varTaxRate,varStorageType,varLocationName, varStokerID, varStokerName,varPur,varPurOrdersID,varPurOrdersSN;
                            ...
                            varPurOrdersID = dgvStockInfo[7, i].Value.ToString();
                            varPurOrdersSN = dgvStockInfo[8, i].Value.ToString();
                            varStockID = dgvStockInfo[2, i].Value.ToString();
                            varSCheckCount = dgvStockInfo[11, i].Value.ToString();
                            varPUnitPrice = dgvStockInfo[12, i].Value.ToString();
                            varTaxRate = dgvStockInfo[13, i].Value.ToString();
                            varStorageType = dgvStockInfo[24, i].Value.ToString();
                            varLocationName = dgvStockInfo[25, i].Value.ToString();
                            varStokerID = dgvStockInfo.Rows[i].Cells[20].Value.ToString();
                            varStokerName = dgvStockInfo.Rows[i ].Cells[21].Value.ToString();
                            varPur = dgvStockInfo.Rows[i].Cells[22].Value.ToString();
                            string[] arr = new string[] { varWareID, varWareName, varSpec, varUnit, varStockID, varSCheckCount, varPUnitPrice,varTaxRate ,varStorageType ,varLocationName , varStokerID, varStokerName,varPur,varPurOrdersID ,varPurOrdersSN };
                            ...
                            C23.StockManage.frmReturnT.getStockInfo[13] = arr[13];
                            C23.StockManage.frmReturnT.getStockInfo[14] = arr[14];
                            this.Close();
                        }
       ...
       private void setStokerData()
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            string varStockCount = dt.Rows[i][10].ToString();
                                            string varPurOrdersID = dt.Rows[i][7].ToString();
                                            string varPurOrdersSN = dt.Rows[i][8].ToString();
                                            string varPCount = dt.Rows[i][9].ToString();
                                            decimal varCheckCount = Decimal.Parse(dt.Rows[i][11].ToString());
                                            decimal varPUnitPrice = Decimal.Parse(dt.Rows[i][12].ToString());
                                            decimal varTaxRate = 17;
                                            string varAcceptor = cboxAcceptor.Text.Trim();
                                            string varStorageType = dt.Rows[i][24].ToString();
                                            string varLocationName = dt.Rows[i][25].ToString();
                                            ...
                                            SqlDataReader sqlread2 = boperate.getread("select RootID,SN,StockCount from tb_Inventory where RootID='" + varID + "' AND SN='" + varSN + "'");
                                            sqlread.Read();
                                            sqlread2.Read();
                                            if (sqlread.HasRows && sqlread2 .HasRows)
                                            {
                                                decimal vargetCheckCount = Convert.ToDecimal(sqlread2["StockCount"]);
                                            ...
                                            SqlDataReader sqlreadupdate2 = boperate.getread("select TotalCount,Tax,NoTax,RootID,SN from tb_PayPart where RootID='" + varID +
                                                "' AND SN='" + varSN + "'");
                                            sqlreadupdate.Read();
                                            sqlreadupdate2.Read();
                                            if (sqlreadupdate.HasRows && sqlreadupdate2 .HasRows )
                                            {
                                                ...
                                                decimal vargetLastTotalCount = Convert.ToDecimal(sqlreadupdate2["TotalCount"]);
                                                decimal vargetLastNoTax = Convert.ToDecimal(sqlreadupdate2["NoTax"]);
                                                decimal vargetLastTax = Convert.ToDecimal(sqlreadupdate2["Tax"]);
                                                decimal varupdateTotalCount, varupdateNoTax, varupdateTax;

                                                if (varTTC  > vargetLastTotalCount)
                                                {
                                                    varupdateTotalCount = vargetTotalCount +varTTC  - vargetLastTotalCount;
                                                    if (varT > vargetLastTax)
                                                    {
                                                        varupdateTax = vargetTax + varT  - vargetLastTax;
                                                        varupdateNoTax = varupdateTotalCount - varupdateTax;
                                                    }
                                                    else
                                                    {

                                                        varupdateTax = vargetTax - (vargetLastTax - varT );
                                                        varupdateNoTax = varupdateTotalCount - varupdateTax;
                                                    }
                                                ...
                                                {
                                                    varupdateTotalCount = vargetTotalCount - (vargetLastTotalCount - varTTC );
                                                    if (varT > vargetLastTax)
                                                    {
                                                        varupdateTax = vargetTax +varT - vargetLastTax;
                                                        varupdateNoTax = varupdateTotalCount - varupdateTax;
                                            ...
                                            #endregion
                                            #region Sc
                                                    decimal varLastSCheckCount, vargetTNSCheckCount,varupdateTNSCheckCount;
                                                    ...
                                                    SqlDataReader sqlreadSc2 = boperate.getread("select StockCount,RootID,SN from tb_Inventory where RootID='" + varID + "' AND SN='" + varSN + "'");
                                                    sqlreadSc.Read();
                                                    sqlreadSc2.Read();
                                                    if (sqlreadSc.HasRows && sqlreadSc2.HasRows)
                                                    {
                                                        vargetTNSCheckCount = Convert.ToDecimal(sqlreadSc["TNSCheckCount"]);
                                                        varLastSCheckCount = Convert.ToDecimal(sqlreadSc2["StockCount"]);
                                                        if (varCheckCount > varLastSCheckCount)
                                                        {
                                                            varupdateTNSCheckCount = vargetTNSCheckCount + (varCheckCount - varLastSCheckCount);
                                                        }
                                                        else
                                                        {
                                                            varupdateTNSCheckCount = vargetTNSCheckCount - (varLastSCheckCount - varCheckCount);
                                                        }
                                                    ...
                                                    #endregion    
                                            #region USCount
                                                        SqlDataReader sqlreadus = boperate.getread("select PurOrdersID,SN,PCount from tb_PurOrders where PurOrdersID='" + varPurOrdersID  +
                                                        ...
                                                        if (sqlreadus.HasRows)
                                                        {
                                                            SqlDataReader sqlreadte = boperate.getread("select TNReCount from tb_TNRe where PurOrdersID='" + varPurOrdersID +
                                                                "'AND PurOrdersSN='" + varPurOrdersSN + "'");
                                                            sqlreadte.Read();
                                                            decimal varupdateUSCount,varupdateTNReaSCheckCount;
                                                            if (sqlreadte.HasRows)
                                                            {
                                                                decimal vargetTNReCount = Convert.ToDecimal(sqlreadte["TNReCount"]);
                                                                decimal vargetPCount = Convert.ToDecimal(sqlreadus["PCount"]);
        ...
        private void tsbtnRecall_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("确定要撤消单据关联吗？", "提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    if (dt.Rows.Count > 0)
                    {
                        boperate.getcom("delete tb_Verify where VerifyID='" + dt.Rows[dgvStockInfo.CurrentCell.RowIndex][7].ToString() + "'");
        ...
        private void tsbtnVerify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    boperate.getcom("insert into tb_Verify (VerifyID) values ('" + dt.Rows[dgvStockInfo.CurrentCell.RowIndex][7].ToString() + "')");
